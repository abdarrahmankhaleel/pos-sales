...
// /*get some cols  table by ppaginaation */
...
function get_cols_row_where($model, $columns_names = array(), $where = array())
{
  $data = $model::select($columns_names)->where($where)->first();
  return $data;
}

function get_cols_row_where2($model, $columns_names = array(), $where = array(), $where2 = array())
{
...
function updateData($model,$dataToUpdate,$where=array()){
