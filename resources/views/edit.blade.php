...
@section('content')
<form method="POST" action="{{ route('updatejob',$data->id) }}">
  @csrf 
    ...
    <div class="form-group">
      ...
      <select name="job_active" id="job_active">
        ...
        <option value="1" @if ( old('job_active',$data['active'])==1 )
          selected
        @endif>yes</option>
        <option @if ( old('job_active',$data['active'])==0 and old('job_active',$data['active'])!="")
        selected
