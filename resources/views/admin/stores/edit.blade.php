  ...
  <div class="col-12">
    <div class="card">
      ...
      <div class="card-body">
        ...
        <form action="{{ route('admin.stores.update',$data['id']) }}" method="post" enctype="multipart/form-data">
          @csrf
            ...
            <div class="form-group">
              ...
              <input name="phone" id="phone" class="form-control" value="{{ old('phone',$data['phone']) }}"  oninvalid="setCustomValidity('من فضلك ادخل هذا الحقل')" onchange="try{setCustomValidity('')}catch(e){}"  >
              @error('phone')
              <span class="text-danger">{{ $message }}</span>
              @enderror
              </div>
            <div class="form-group">
              ...
              <input name="address" id="address" class="form-control" value="{{ old('address',$data['address']) }}"  oninvalid="setCustomValidity('من فضلك ادخل هذا الحقل')" onchange="try{setCustomValidity('')}catch(e){}"  >
              @error('address')
              <span class="text-danger">{{ $message }}</span>
              @enderror
              </div>
        ...
        @endif
        <a href="{{ route('admin.stores.index') }}" class="btn btn-sm btn-success">الغاء</a>
      </div>
    </div>
  </div>
</div>
